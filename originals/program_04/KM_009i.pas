    ...
    Begin
      For j:=-Bn To Bn Do {in y-Richtung}
        Begin
          For k:=-Bn To Bn Do {in z-Richtung}
            Begin
              Bx[i,j,k] := -Sgn(i)/(i*i+j*j+k*k+1);
              If i=0 Then Bx[i,j,k] := 0; {Telsa}
              By[i,j,k] := 10/(i*i+j*j+k*k+1); {Telsa}
              Bz[i,j,k] := -Sgn(k)/(i*i+j*j+k*k+1);
              If k=0 Then Bz[i,j,k] := 0; {Telsa}
              OrtBx[i,j,k] := i*Bsw;
              OrtBy[i,j,k] := j*Bsw;
              OrtBz[i,j,k] := k*Bsw;
...
Begin
  ...
  qwill := 1;
  om := 2*pi*MEI/qwill; {Ladung und der Kreisfrequenz in der Magnetfeld-Emulationsspule}
...
{ Writeln('Gesamtes Feld am Aufpunkt. : ',Hx:12:7,', ',Hy:12:7,', ',Hz:12:7,' A/m'); }
  Hxkl := 0;
  Hzkl := 0; {klassische Berechnung im Vgl.}
  Nenner := Sqrt(MEro*MEro+(MEyo-sy)*(MEyo-sy));
...
Begin
  Writeln;
  Writeln('Magnetfeld Emulations-Spulenpaar nach *1 von S.5');
      ...
      For j2:=-Bn To Bn Do {in y-Richtung}
        Begin
          For k2:=-Bn To Bn Do {in z-Richtung}
            Begin
              OrtBx[i2,j2,k2] := i2*Bsw;
              sx := OrtBx[i2,j2,k2];
              OrtBy[i2,j2,k2] := j2*Bsw;
              sy := OrtBy[i2,j2,k2];
              OrtBz[i2,j2,k2] := k2*Bsw;
              sz := OrtBz[i2,j2,k2];
              Berechne_Hges;
...
Wait; }
      ...
      Write('.');
    End;
  Writeln(' Obere Spule ist durchgerechnet.');
...
Begin
  ...
  For i:=0 To 360 Do
    merk := merk+Sqr(PSIF[i]-B1*sin(i/360*2*pi));
  QuadSum1 := merk;
End;
...
Begin
  merk := 0; {'i' ist Laufvariable für den Winkel, anzugeben in Grad}
  For i:=0 To 360 Do
    merk := merk+Sqr(PSIF[i]-Koeff1*sin(i/360*2*pi)-Koeff2*sin(2*i/360*2*pi)-
...
{Jetzt B1 anpassen über die Minimierung der Abweichungsquadrate:}
  Repeat
    ...
    B1 := B1/1.01;
    weiter := false;
    If QSminus<QSmitte Then
      Begin
        B1 := 0.99*B1;
        weiter := true;
      End;
    If QSplus<QSmitte Then
      Begin
        B1 := 1.01*B1;
        weiter := true;
      End;
...
{QuadSummen berechnen:}
    ...
    Q1p := QuadSum3(B1*1.01,B2,B3);
    Q1m := QuadSum3(B1*0.99,B2,B3);
    Q2p := QuadSum3(B1,B2*1.01,B3);
    Q2m := QuadSum3(B1,B2*0.99,B3);
    Q3p := QuadSum3(B1,B2,B3*1.01);
    Q3m := QuadSum3(B1,B2,B3*0.99);
{Kleinste QuadSumme suchen:}
    QSminimum := Qanf;
    If Q1p<QSminimum Then QSminimum := Q1p;
    If Q1m<QSminimum Then QSminimum := Q1m;
    If Q2p<QSminimum Then QSminimum := Q2p;
    If Q2m<QSminimum Then QSminimum := Q2m;
    If Q3p<QSminimum Then QSminimum := Q3p;
    If Q3m<QSminimum Then QSminimum := Q3m;
{Koeffizienten zur kleinsten QuadSumme einstellen:}
    weiter := false;
    If Q1p=QSminimum Then
      Begin
        B1 := B1*1.01;
        weiter := true;
      End;
    If Q1m=QSminimum Then
      Begin
        B1 := B1*0.99;
        weiter := true;
      End;
    If Q2p=QSminimum Then
      Begin
        B2 := B2*1.01;
        weiter := true;
      End;
    If Q2m=QSminimum Then
      Begin
        B2 := B2*0.99;
        weiter := true;
      End;
    If Q3p=QSminimum Then
      Begin
        B3 := B3*1.01;
        weiter := true;
      End;
    If Q3m=QSminimum Then
      Begin
        B3 := B3*0.99;
        weiter := true;
      End;
...
{Startwerte für B4 suchen:}
  B4 := 0;
  B4 := +B1/40;
  QSplus := QuadSum5(B1,B2,B3,B4,0);
  B4 := -B1/40;
  QSminus := QuadSum5(B1,B2,B3,B4,0);
  If QSplus<QSminus Then B4 := +B1/40;
  If QSminus<QSplus Then B4 := -B1/40;
{Startwerte für B5 suchen:}
  B5 := 0;
  B5 := +B1/40;
  QSplus := QuadSum5(B1,B2,B3,B4,B5);
  B5 := -B1/40;
  QSminus := QuadSum5(B1,B2,B3,B4,B5);
...
{QuadSummen berechnen:}
    ...
    Q1p := QuadSum5(B1*1.01,B2,B3,B4,B5);
    Q1m := QuadSum5(B1*0.99,B2,B3,B4,B5);
    Q2p := QuadSum5(B1,B2*1.01,B3,B4,B5);
    Q2m := QuadSum5(B1,B2*0.99,B3,B4,B5);
    Q3p := QuadSum5(B1,B2,B3*1.01,B4,B5);
    Q3m := QuadSum5(B1,B2,B3*0.99,B4,B5);
    Q4p := QuadSum5(B1,B2,B3,B4*1.01,B5);
    Q4m := QuadSum5(B1,B2,B3,B4*0.99,B5);
    Q5p := QuadSum5(B1,B2,B3,B4,B5*1.01);
    Q5m := QuadSum5(B1,B2,B3,B4,B5*0.99);
{Kleinste QuadSumme suchen:}
    QSminimum := Qanf;
    If Q1p<QSminimum Then QSminimum := Q1p;
    If Q1m<QSminimum Then QSminimum := Q1m;
    ...
    If Q3p<QSminimum Then QSminimum := Q3p;
    If Q3m<QSminimum Then QSminimum := Q3m;
    If Q4p<QSminimum Then QSminimum := Q4p;
    If Q4m<QSminimum Then QSminimum := Q4m;
    If Q5p<QSminimum Then QSminimum := Q5p;
    If Q5m<QSminimum Then QSminimum := Q5m;
{Koeffizienten zur kleinsten QuadSumme einstellen:}
    weiter := false;
    If Q1p=QSminimum Then
      Begin
    ...
    If Q3m=QSminimum Then
      Begin
        ...
        weiter := true;
      End;
    If Q4p=QSminimum Then
      Begin
        B4 := B4*1.01;
        weiter := true;
      End;
    If Q4m=QSminimum Then
      Begin
        B4 := B4*0.99;
        weiter := true;
      End;
    If Q5p=QSminimum Then
      Begin
        B5 := B5*1.01;
        weiter := true;
      End;
    If Q5m=QSminimum Then
      Begin
        B5 := B5*0.99;
        weiter := true;
      End;
...
Begin
  merk := 0; {'i' ist Laufvariable für den Winkel, anzugeben in Grad}
  For i:=0 To 360 Do
    merk := merk+Sqr(PSIF[i]-B1lok*sin((i+phaselok)/360*2*pi));
  QuadSum2 := merk;
End;
Begin
  Assign(fin,'PSIF.DAT');
  ...
  For i:=0 To 360 Do
    ...
    End;
  Writeln('Startwert von B1: ',B1);
  Wait;
  phase := 0;
  QSalt := QuadSum2(B1,phase);
  jmerk := Round(phase); {Startwert für Grundschwingung "phase" suchen}
  For j:=1 To 360 Do
    Begin
      phase := j;
      QSneu := QuadSum2(B1,phase);
      If QSneu<QSalt Then
        Begin
          QSalt := QSneu;
          jmerk := j;
{ Writeln(phase,' => ',QSalt); Wait; }
      ...
      phase := jmerk;
    End;
  Writeln('Startwert von phase: ',phase);
  Wait;
...
{Koeffizienten zur kleinsten QuadSumme einstellen:}
    weiter := false;
    If QB1plus=QSminimum Then
      Begin
        B1 := B1*1.0001;
        weiter := true;
      End;
    If QB1minus=QSminimum Then
      Begin
        B1 := B1*0.9999;
        weiter := true;
      End;
    If Qphaseplus=QSminimum Then
      Begin
        phase := phase*1.0001;
        weiter := true;
      End;
    If Qphaseminus=QSminimum Then
      Begin
        phase := phase*0.9999;
        weiter := true;
      End;
...
{Ort und Richtungsvektoren der Leiterschleifen-Elemente feststellen, Feld am Aufpunkt ausrechnen nach Biot-Savart:}
  ...
  For i:=1 To AnzLSE Do
    Begin
      sx := SpTx[i];
      sy := SpTy[i];
      sz := SpTz[i]; {Orte der Leiterschleifen-Elemente}
      dsx := dSTx[i];
      dsy := dSTy[i];
      dsz := dSTz[i]; {Richtungsvektoren der Leiterschleifen-Elemente}
      smrx := sx-rx;
      smry := sy-ry;
      smrz := sz-rz; {Differenzen für das nachfolgende Kreuzprodukt}
      krpx := dsy*smrz-dsz*smry;
      krpy := dsz*smrx-dsx*smrz;
      krpz := dsx*smry-dsy*smrx; {Kreuzprodukt}
      ...
      Hgesx := Hgesx+dHx;
      Hgesy := Hgesy+dHy;
      Hgesz := Hgesz+dHz; {Summation der Feldelemente}
    End; {Vorzeichen in der nachfolgenden Zeile gemäß technischer Stromrichtung.}
  BTx := -muo*Hgesx*Nturbo;
  BTy := -muo*Hgesy*Nturbo;
  BTz := -muo*Hgesz*Nturbo;
End;
...
{Ort und Richtungsvektoren der Leiterschleifen-Elemente feststellen, Feld am Aufpunkt ausrechnen nach Biot-Savart:}
  ...
  For i:=1 To AnzLSE Do
    Begin
      sx := SpIx[i];
      sy := SpIy[i];
      sz := SpIz[i]; {Orte der Leiterschleifen-Elemente}
      dsx := dSIx[i];
      dsy := dSIy[i];
      dsz := dSIz[i]; {Richtungsvektoren der Leiterschleifen-Elemente}
      smrx := sx-rx;
      smry := sy-ry;
      smrz := sz-rz; {Differenzen für das nachfolgende Kreuzprodukt}
      krpx := dsy*smrz-dsz*smry;
      ...
      Hgesz := Hgesz+dHz; {Summation der Feldelemente}
    End; {Vorzeichen in der nachfolgenden Zeile gemäß technischer Stromrichtung.}
  BIx := -muo*Hgesx*Ninput;
  BIy := -muo*Hgesy*Ninput;
  BIz := -muo*Hgesz;
End;
...
{Es folgt die Berechnung der Magnetfeld-Stärke der Input- und Turbo- Spule am Ort des Leiterschleifen-Elements}
      ...
      Bxlok := BIx+BTx;
      Bylok := BIy+BTy;
      Bzlok := BIz+BTz; {lokales B-feld der Input- und der Turbo- Spule am Ort des Leiterschleifen-Elements}
...
{Zuerst die Parameter:}
  ...
  If x<>Spsw Then
    Begin
      schonda := false;
      Writeln(' Spsw geaendert');
    End;
  Readln(fin,n);
  If n<>xo Then
    Begin
      schonda := false;
      Writeln(' xo geaendert');
    End;
  Readln(fin,n);
  If n<>yo Then
    Begin
      schonda := false;
      Writeln(' yo geaendert');
    End;
  Readln(fin,n);
  If n<>zo Then
    Begin
      schonda := false;
      Writeln(' zo geaendert');
    End;
  Readln(fin,n);
  If n<>Ninput Then
    Begin
      schonda := false;
      Writeln('Ninput geaendert');
    End;
  Readln(fin,n);
  If n<>Nturbo Then
    Begin
      schonda := false;
      Writeln('Nturbo geaendert');
    End;
  Readln(fin,x);
  If x<>Bsw Then
    Begin
      schonda := false;
      Writeln(' Bsw geaendert');
    End;
  Readln(fin,x);
  If x<>MEyo Then
    Begin
      schonda := false;
      Writeln(' MEyo geaendert');
    End;
  Readln(fin,x);
  If x<>MEro Then
    Begin
      schonda := false;
      Writeln(' MEro geaendert');
    End;
  Readln(fin,x);
  If x<>MEI Then
    Begin
      schonda := false;
      Writeln(' MEI geaendert');
    End;
  ...
  If i>=Pulsdauer Then {Getriggerte Pulse im Betrieb geben}
    Begin
      If (i>=iumk+Phasenshift)And(i<=iumk+Pulsdauer+Phasenshift) Then {Hier wird das Trigger-Signal am oberen Umkehrpunkt festgemacht.}
        Begin
          Umerk := UAmpl;
        End; {Spannung anlegen} {Alternativ könnte man ihn z.B. auch am Nulldurchgang festmachen.}
    End;
...
{ Naturkonstanten:}
  ...
  LiGe := Sqrt(1/muo/epo){m/s};
  Writeln('Lichtgeschwindigkeit c = ',LiGe, ' m/s');
...
{ Die beiden Spulen, vgl. Zeichnung *2 von S.1 :} {Die Spulen werden nach Vorgabe der Geometrieparameter automatisch vernetzt}
  ...
  xo := 0;
  yo := 6;
  zo := 5; {Angaben in Vielfachen von Spsw} {Geometrieparameter nach Zeichnung*2 von S.1}
  ...
  If nebeninput*ueberinput<>Ninput Then
    Begin
      Writeln;
      Writeln('Windungszahl falsch: So kann man die Input-Spule nicht anordnen !');
      Wait;
      Wait;
      Halt;
    End;
  If nebenturbo*ueberturbo<>Nturbo Then
    Begin
      Writeln;
      Writeln('Windungszahl falsch: So kann man die Turbo-Spule nicht anordnen !');
      Wait;
      Wait;
      Halt;
    End;
...
{ Sonstige (zur Eingabe):}
  ...
  Uc := 0;{Volt}
  Il := 0; {Ampere} {Anfangsbedingung elektrisch - Kondensatorspannung = 0, Kein Spulenstrom}
...
{ Abgeleitete Parameter. Die Größen werden aus den obigen Parametern berechnet, es ist keine Eingabe möglich:}
  ...
  BreiteI := nebeninput*DD;
  HoeheI := ueberinput*DD; {Breite und Höhe des Input-Spulenlörpers}
  BreiteT := nebenturbo*DD;
  HoeheT := ueberturbo*DD; {Breite und Höhe des Turbo-Spulenlörpers}
...
{ Anzeige der Werte:}
  ...
  If ((PlotEnde-PlotAnfang)/PlotStep)>AnzPmax Then
    Begin
      Writeln;
      Writeln('FEHLER: Zu viele Datenpunkte.');
      Writeln('So viele Datenpunkte koennen in Excel nicht dargestellt werden.');
      Writeln('=> Berechnung wird hier GESTOPPT.');
      Wait;
      Wait;
      Halt;
    End;
...
1.0 Ampere}
  ...
  merk := Sqrt((2*yo*Spsw*2*zo*Spsw)+Sqr(xo*Spsw));
  merk := merk*merk*merk;
...
{Teil 2: Ausprobieren der Flußberechnung durch die beiden Spulen unter Magnet-Drehung (führt später zur induz. Spannung):}
  If Not(schonda) Then
    Begin
      ...
      Gesamtfluss_durch_Input_Spule;
      Writeln('Gesamtfluss durch Input-Spule: ',PsiGES,' T*m^2');
      Magnet_drehen(01); {Drehwinkel in Grad angeben, 0...360}
      Gesamtfluss_durch_Input_Spule;
      Writeln('Gesamtfluss durch Input-Spule: ',PsiGES,' T*m^2');
      Writeln('-----------------------');
      Magnet_drehen(00); {Drehwinkel in Grad angeben, 0...360}
      Gesamtfluss_durch_Turbo_Spule;
      Writeln('Gesamtfluss durch Turbo-Spule: ',PsiGES,' T*m^2');
      Magnet_drehen(01); {Drehwinkel in Grad angeben, 0...360}
      Gesamtfluss_durch_Turbo_Spule;
      Writeln('Gesamtfluss durch Turbo-Spule: ',PsiGES,' T*m^2');
      Writeln('-----------------------');
    ...
    Begin
      ...
      For i:= 0 To 360 Do {Zuerst probiere ich's mit der Input-Spule}
        Begin
          ...
          Magnet_drehen(phi[i]);
          Gesamtfluss_durch_Input_Spule; {setzt auf "PsiGES" den Ergebnis-Wert ab.}
          ...
          KG[i] := 0;
          KH[i] := PSIinput[i];
          KI[i] := UindInput[i]; {Zur Excel-Ausgabe weiterleiten}
        End;
      Writeln('---------------------------------');
      ...
      For i:= 0 To 360 Do {Danach probiere ich's auch noch mit der Turbo-Spule}
        Begin
          phi[i] := i; {Angabe in Grad}
          Magnet_drehen(phi[i]);
          Gesamtfluss_durch_Turbo_Spule; {setzt auf "PsiGES" den Ergebnis-Wert ab.}
          ...
          KJ[i] := 0;
          KK[i] := PSIturbo[i];
          KL[i] := Uindturbo[i]; {Zur Excel-Ausgabe weiterleiten}
          KM[i] := 0;
          KN[i] := KN[i]; {Am Ende noch zwei Leerspalten}
        End;
{Jetzt muß man das Signal noch gegen Rauschen glätten:}
      FourierDatenspeicherung(PSIturbo);
      FourierEntwicklung;
      B1T := B1;
      B2T := B2;
      B3T := B3;
...
{**}
      writeln(B1T:13,', ',B2T:13,', ',B3T:13,', ',B4T:13,', ',B5T:13);
      Wait;
      FourierDatenspeicherung(PSIinput);
      FourierEntwicklung;
      B1I := B1;
      B2I := B2;
      B3I := B3;
...
{Daher entwickele ich jetzt auch eine Fourier-Reihe zur Beschleunigung der Drehmoments-Berechnung:}
  If Not(schonda) Then
    Begin
      qpoT := 1;
      qpoI := 0; {Schnell-Berechnungs-Kalibrierung nur für Turbo-Spule, 1A, aber ohne Input-Spule}
      ...
      For i:=0 To 360 Do
        Begin {Das gesamt-Drehmoment, das der Magnet im Feld der beiden Spulen (Input&Turbo) aufnimmt.}
          ...
          Write('.'); {Writeln(KN[i]);}
        End;
      FourierDatenspeicherung(KN);
      SinusEntwicklung_fuer_Drehmoment;
...
{ Zuerst: Anfangsbedingungen einsetzen:}
  phio := 0;
  phipo := omAn; {phippo:=0;} {Anfangsbedingungen der Magnetrotation (mechanisch)}
{Starten mit gegebener Anfangs-Winkelgeschwindigkeit ist vorgesehen.}
  qoT := CT*Uc;
  qpoT := Il; {qppoT:=0;} {Anfangsbedingungen Turbo-Spule (elektrisch)}
{Ein geladener Kondensator im Schwingkreis der Turbo-Spule ist vorgesehen.}
  qoI := 0;
  qpoI := 0;
  qppoI := 0; {Anfangsbedingungen Input-Spule (elektrisch), zunächst inaktiv}
...
{Für den neuen Schritt, wird der alte Vorgänger-Schritt zum vorletzten Schritt heruntergezählt:}
      phim := phio;
      phipm := phipo; {phippm:=phippo;} {Drehbewegung}
      qmT := qoT;
      qpmT := qpoT; {qppmT:=qppoT;} {Turbo-Spule}
      qmI := qoI;
      qpmI := qpoI;
      qppmI := qppoI; {Input-Spule}
...
{GG}
      If i=1 Then cr := crAnfang; {Geschwindigkeits-proportionaler Reibungs-Koeffizient}
{GG}
      If i>1 Then cr := Reibung_nachregeln; {Dieser wird nachgeregelt, um eine konstante Drehzahl
einzustellen für stabilen Betrieb der Maschine.}
{GG}
      If phipo>0 Then phippo := phippo-cr*phipm/J; {Die neg. Beschleunigung wirkt immer der Geschwindigkeit entgegen.}
{GG}
      If phipo=0 Then phippo := phippo;
{GG}
      If phipo<0 Then phippo := phippo+cr*phipm/J; {Die neg. Beschleunigung wirkt immer der Geschwindigkeit entgegen.}
...
{GG}
      Ereib := Ereib+Preib*dt; {Gesamte über Reibung entnommene Leistung}
{Dann die Turbo-Spule. Gedämpfte elektrische Schwingung, dazu induzierte Spannung aufgrund Magnet-Drehung:}
{FF}
      qppoT := -1/(LT*CT)*qmT-(RT+Rlast)/LT*qpmT; {Dgl. der gedämpfte Schwingung.}
...
{Ggf. muß ich jetzt einen Plot-Punkt für's Excel ausgeben:}
        ...
        Begin
          If ((i-PlotAnfang)Mod(PlotStep))=0 Then
            Begin
              ...
              Q[znr] := qoT;
              Qp[znr] := qpoT;
              Qpp[znr] := qppoT; {Turbo-Spule, im Array (und nur dort) ohne Index "T".}
              QI[znr] := qoI;
              QpI[znr] := qpoI;
              QppI[znr] := qppoI; {Input-Spule}
              phi[znr] := phio;
              phip[znr] := phipo;
              phipp[znr] := phippo; {Drehung des Magneten}
              KK[znr] := FlussT(phio);
              KL[znr] := FlussI(phio); {Magnetischer Fluß durch die Spulen}
              KM[znr] := UinduzT;
              KN[znr] := UinduzI; {In den Spulen induzierte Spannung}
    ...
    End;
  Writeln;
  Writeln('Anzahl Datenpunkte fuer Excel-Plot: LPP = ',LPP);
  Writeln;
  Writeln('Anzeigen der Amplituden der Auslegung: (nicht Effektivwerte, sondern Spitze)');
  ...
  Writeln;
  Writeln('Fertig gerechnet -> Adele.');
  Wait;
  Wait;
End.
