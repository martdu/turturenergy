...
Var epo,muo : Double; {Naturkonstanten}
  ...
  x1,x2,v1,v2 : Array [0..200000] Of Double;
  {Zeit,Orte,Gescheindigkeiten der Ladungen}
...
{ Initialisierung - Vorgabe der Werte: }
  D := 0;
  r := 0; {Zur Vermeidung von Delphi-Meldungen}
  ...
  ianf := 0;
  iend := 100000; {Nummer des ersten und letzten Zeitschritts}
...
{ Teil 2: Test -> anharmonische Schwingung, mit elektr. Ladung, oder Magnet: STATISCH !}
  ...
  Q1 := 2.01E-5{C};
  Q2 := 2.01E-5{C}; {Ladungen der beiden Körper}
  D := 0.20;{N/m} {Federkonstante}
  RLL := 6.0;{m} {Ruhelage-Länge der Feder} {Ruhelage-Positionen bei +/-RLL/2}
  x1[0] := -3.8;
  x2[0] := +3.8; {Startpositionen der Massen mit Vorspannung}
  v1[0] := 00.00;
  v2[0] := 00.00; {Startgeschwindigk. der schwingenden Massen}
  ...
  Repeat
    ...
    If FL<=1E-20 Then
      Begin
        ...
        Writeln(
     'Daten wurden gespeichert in "XLS-Nr-02.DAT", dann Abbruch der Berechnung.'
        );
        Wait;
        Halt;
      End;
    If FL>1E-20 Then
      ...
      End;
{Kontrolle:}
    If i=1 Then Writeln('El.-kraefte: ',FEL1,' und ',FEL2,' Newton');
{Kontrolle:}
    If i=1 Then Writeln('Federkraefte: ',FK1, ' und ',FK2,' Newton');
    a1 := (FK1+FEL1)/m1;
    a2 := (FK2+FEL2)/m2; {Beschleunigungen der beiden Körper}
    v1[i] := v1[i-1]+a1*delt;
    {So verändert die Beschl. die Geschw. des Körpers 1}
    v2[i] := v2[i-1]+a2*delt;
    {So verändert die Beschl. die Geschw. des Körpers 2}
    x1[i] := x1[i-1]+v1[i-1]*delt;
    {So verändert die Geschw. die Pos. des Körpers 1}
    x2[i] := x2[i-1]+v2[i-1]*delt;
    {So verändert die Geschw. die Pos. des Körpers 2}
  Until i=iend;
...
{ Teil 3: Test -> Mit endlicher Propagationsgeschwindigkeit der Felder}
  ...
  Q1 := 3E-5{C};
  Q2 := 3E-5{C}; {Ladungen der beiden Körper}
  D := 2.7;{N/m} {Federkonstante}
  RLL := 8.0;{m} {Ruhelage-Länge der Feder} {Ruhelage-Positionen bei +/-RLL/2}
  x1[0] := -3.0;
  x2[0] := +3.0; {Startpositionen der Massen mit Vorspannung}
  v1[0] := 00.00;
  v2[0] := 00.00; {Startgeschwindigk. der schwingenden Massen}
  Ukp := x2[0];
  UkpAlt := Ukp;
  unten := true;
  neu := true; {Vorgabe des ersten unteren Umkehrpunktes}
...
{ Jetzt beginnt die schrittweise Ermittlung der Bewegung:}
  Repeat
    ...
    FL := x2[i-1]-x1[i-1]; {Federlänge}
    FK1 := (FL-RLL)*D;
    {Federkraft: pos. Kraft zieht nach rechts, neg. Kraft nach links}
    FK2 := (RLL-FL)*D;
    {Federkraft: pos. Kraft zieht nach rechts, neg. Kraft nach links}
{ Berechnung der Feld-lauf-dauer, Feld-lauf-strecke und daraus Feld-stärke}
    ...
    FEL2 := 0;
    tj := i;
    ts := i; {ich nehme i als Maß für die Zeit}
...
{ Writeln('tj=',tj*delt:9:5,' ts=',ts*delt:9:5,'=>',diff:9:5); }
    ...
    If diff>=0 Then
      {Vor Beginn beim Zeitpunkt Null waren die Körper am Ausgangspunkt ruhend}
      Begin
        r := x2[Round(tj)]-x1[0];
{ Writeln('diff>=0; r=',r); }
      End;
...
{ Writeln(' und tr=',tr:13:9,' und r=',r:13:9); }
    ...
    If r<=1E-10 Then
      Begin
        ...
        Writeln(
         'Daten wurden gespeichert in "XV-03.DAT", dann Abbruch der Berechnung.'
        );
        Wait;
        Halt;
      End;
